...
/**
 * Чтобы пройтись по проекту и вытащить новые непереведенные сообщения в базу, введите:
 * ./yii translate
 *
 * Конфиг файл настроен на работу с БД, для переключения на файлы в параметре format конфига указать php
 *
...
EOD;
        ...
        if (file_put_contents($filePath, $content) === false) {
            $this->stdout("Файл конфигурации НЕ был создан: '{$filePath}'.\n\n", Console::FG_RED);
            return ExitCode::UNSPECIFIED_ERROR;
        }

        $this->stdout("Создан файл конфигурации: '{$filePath}'.\n\n", Console::FG_GREEN);
        return ExitCode::OK;
    }
    ...
    public function actionConfigTemplate($filePath)
    {
        ...
        if (file_exists($filePath)) {
            if (!$this->confirm("Файл '{$filePath}' уже существует. Вы хотите перезаписать его?")) {
                return ExitCode::OK;
            }
        }

        if (!copy(Yii::getAlias('@yii/views/messageConfig.php'), $filePath)) {
            $this->stdout("Шаблон конфигурационного файла НЕ был создан в '{$filePath}'.\n\n", Console::FG_RED);
            return ExitCode::UNSPECIFIED_ERROR;
        }

        $this->stdout("Шаблон конфигурационного файла создан по адресу '{$filePath}'.\n\n", Console::FG_GREEN);
        return ExitCode::OK;
    }
     ...
     * @throws Exception on failure.
     */
    public function actionExtract($configFile = 'console/config/translate.php')
    {
        ...
        $this->stdout("Обработано файлов: " . count($files). "\n");
    ...
    protected function saveMessagesToDb($messages, $db, $sourceMessageTable, $messageTable, $removeUnused, $languages, $markUnused)
    {
        ...
        $this->stdout("Вставка новых сообщений...\n");
        $savedFlag = false;
        ...
        foreach ($new as $category => $msgs) {
            ...
            $this->stdout("Добавлено " . count($msgs). " новых сообщений из категории " .$category. "\n");
        }
        ...
        $this->stdout($savedFlag ? "сохранено.\n" : "Ничего для сохранения.\n");
        $this->stdout($removeUnused ? 'Удаление устаревших сообщений...' : 'Обновление устаревших сообщений...');

        if (empty($obsolete)) {
            $this->stdout("Ничего устарелого ... пропущено.\n");
            return;
        }
        ...
        if ($removeUnused) {
            ...
            $this->stdout("обновлено.\n");
        } else {
            $this->stdout("остался нетронутым.\n");
        }
    }
    ...
    {
//        $this->stdout('Извлечение сообщений из ');
...
//        $this->stdout("\n");
                ...
                if ($this->tokensEqual(')', $token)) {
                    ...
                    if ($pendingParenthesisCount === 0) {
                        ...
                        if (isset($buffer[0][0], $buffer[1], $buffer[2][0]) && $buffer[0][0] === T_CONSTANT_ENCAPSED_STRING && $buffer[1] === ',' && $buffer[2][0] === T_CONSTANT_ENCAPSED_STRING) {
                            ...
                            $this->stdout("$skipping $line. Убедитесь, что категории и сообщения являются статическими строками.\n");
                        }
    ...
    protected function saveMessagesToPHP($messages, $dirName, $overwrite, $removeUnused, $sort, $markUnused)
    {
        foreach ($messages as $category => $msgs) {
            ...
            $this->stdout("Сохранение сообщений в $coloredFileName...\n");
    ...
    protected function saveMessagesCategoryToPHP($messages, $fileName, $overwrite, $removeUnused, $sort, $category, $markUnused)
    {
        if (is_file($fileName)) {
            ...
            if (array_keys($existingMessages) === $messages && (!$sort || array_keys($rawExistingMessages) === $messages)) {
                $this->stdout("Ничего нового в категории \"$category\" ... Нечего сохранять.\n\n", Console::FG_GREEN);
                return ExitCode::OK;
            }
...
EOD;
        ...
        if (file_put_contents($fileName, $content) === false) {
            $this->stdout("Перевод не был сохранен.\n\n", Console::FG_RED);
            return ExitCode::UNSPECIFIED_ERROR;
        }

        $this->stdout("Перевод сохранен.\n\n", Console::FG_GREEN);
        return ExitCode::OK;
    }
    ...
    protected function saveMessagesToPO($messages, $dirName, $overwrite, $removeUnused, $sort, $catalog, $markUnused)
    {
        ...
        $this->stdout("Сохранение сообщений в $file...\n");
        ...
        foreach ($messages as $category => $msgs) {
            ...
            if (is_file($file)) {
                ...
                if (array_keys($existingMessages) == $msgs) {
                    $this->stdout("Ничего нового в категории \"$category\" ...\n");
            ...
            $this->stdout("Категория \"$category\" объединена.\n");
